...
export function updateStreak(
  ...
  if (!streak.lastSolvedDate) {
    const current = 1
    return { current, best: Math.max(streak.best, current), lastSolvedDate: today }
  }
  ...
  const current = 1
  return { current, best: Math.max(streak.best, current, streak.best), lastSolvedDate: today }
}
